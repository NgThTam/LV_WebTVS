...
fetch("http://localhost:3000/api/v1/books")
	...
	.then((books) => {
		fetch("http://localhost:3000/api/v1/categories")
			.then((res) => res.json())
			.then((categories) => {
				const selectCate = document.getElementById("catesearchSelect");
				const cateSele = categories.map(
					(cate) => `<option value="${cate.IDc}">${cate.NameC}</option>`
				);
				cateSele.unshift(`<option value="All">All</option>`);
				selectCate.innerHTML = cateSele.join(" ");
				selectCate.addEventListener("change", () => {
					const namecatebook = document.querySelector(".js_name_cate_book");
					const findname = categories.find((ct) => ct.IDc == selectCate.value);
					if (findname) {
						namecatebook.innerHTML = `${findname.NameC}`;
					} else {
						namecatebook.innerHTML = "All";
					}
				});
			});
		return books;
	...
	.then((v1books) => {
		fetch("http://localhost:3000/api/v1/book_category")
			...
			.then((bookcategorys) => {
				selectCate.addEventListener("change", () => {
					let books;
					if (selectCate.value == "All") {
						books = v1books;
					} else {
						let allBcate = bookcategorys.filter(
							(Bcate) => Bcate.IDc == selectCate.value
						);
						let cateBooks = allBcate.map((acate) => {
							let bookc = v1books.find((v1B) => v1B.IDb == acate.IDb);
							return bookc;
						});
						books = cateBooks;
					}
					...
					textYear.forEach((y) => {
						y.addEventListener("click", (e) => {
							const newBooks = filterY(books, y.innerText);
							const newListB = [];
							newBooks.forEach((book, indx) => {
								if (indx < 12) {
									newListB.push(book);
							...
							const abookyear1 = newListB.map((newbook) => {
								return renderBook(newbook);
							});
							showBook.innerHTML = abookyear1.join(" ");
							// get ID => detail
							const abooks = document.querySelectorAll(".js_lbk");
							abooks.forEach((abook) => {
								abook.addEventListener("click", (e) => {
					...
					publisers.forEach((pub) => {
						pub.addEventListener("click", () => {
							const newPubs = filterX(books, pub.innerText);
							const abookoke2 = newPubs.map((newpub) => {
								return renderBook(newpub);
							});
							showBook.innerHTML = abookoke2.join(" ");
							// get ID => detail
							const abooks = document.querySelectorAll(".js_lbk");
							abooks.forEach((abook) => {
								abook.addEventListener("click", (e) => {
					...
					authors.forEach((author) => {
						author.addEventListener("click", () => {
							const newAuthors = filterA(books, author.innerText);
							const abook = newAuthors.map((newAuthor) => {
								return renderBook(newAuthor);
							});
							showBook.innerHTML = abook.join(" ");
					...
					fetch("http://localhost:3000/api/v1/ratings")
						...
						.then((ratings) => {
							...
							const clickStars = document.querySelectorAll(
								".js_menu_ratings > li"
							);
							clickStars.forEach((clickstar) => {
								clickstar.addEventListener("click", () => {
